globals [
  w-range ;Range of initial weights
  b-range ;Range of initial biases
  cone-length ;Size of cone
]
breed [Robots Robot]
...
robots-own [
  ...
  b ;Biases
  w ;Weights
  b0 b1 b2 ;Biases of each layer
  w0 w1 w2 ;Weights of each layer
  w0.0 w0.1 w0.2 w0.3 w0.4 ;Weights of each node
  w1.0 w1.1 w1.2 w1.3 w1.4 ;weights of each node
  w2.0 w2.1 w2.2 w2.3 w2.4 ;Weights of each node
]
...
to setup
  ...
  drawGrid
  set-default-shape robots "dot"
  set-default-shape projectiles "default"
  create-robots 1 [set size 6 set color green setxy random min-pxcor random-ycor]
  create-robots 1 [set size 6 set color orange setxy random max-pxcor random-ycor]
  ask robots [set EnemyNear? 0 set ProjectileNear? 0 set ProjectileFired? 0 set Vision 20 set lives 3]
  init-genomes
end
...
to robotBehavior
  ask robots [ask patches in-cone cone-length vision [set pcolor yellow]]
  ask robots [ifelse any? other robots in-cone cone-length vision [set EnemyNear? 1]
    [set EnemyNear? 0]
    ifelse any? projectiles in-cone cone-length vision [set ProjectileNear? 1]
    [set ProjectileNear? 0]
    if lives = 0 [die]
    set inputs (list EnemyNear? ProjectileNear? ProjectileFired? Vision)]
end

to projectileBehavior
 ask projectiles [
    fd 1
    if pxcor = max-pxcor or pycor = max-pycor [die]
  ]
  ask projectiles [
    if count robots-here with [ProjectileFired? = 0] = 1 [ask robots-here [set lives lives - 1] die]
  ]
end
...
;Adds each value in a list to its corresponding value in antoher list
;a - weighted sum of the weights and values of input nodes
;biases - values to add or subtract to the weighted sums
to-report matrix-addition [a biases]
  let output []
  let i 0
  while [i < length a] [
    let q item i a
    let r item i biases
    set output lput (q + r) output
    set i i + 1
  ]
  report output
end

;Finds a list of weighted sums of two arrays
;a - values of input nodes
;weights - list of lists of weights for each output node
to-report matrix-multiplication [a weights]
  let i 0
  let output []
  while [i < length weights] [
    let tmp-value dot-product a (item i weights)
    set output lput tmp-value output
    set i i + 1
  ]
  report output
end

;Finds the dot-product/weighted sum of two lists
;a - values of the input nodes
;weights - weights of each connection for 1 output node
to-report dot-product [a weights]
  let i 0
  let output 0
  while [i < length a] [
    let q item i a
    let r item i weights
    set output output + q * r
    set i i + 1
  ]
  report output
end

;Squishes numbers in a range from 0 to 1
;Used for comparing values of nodes
;x - Any number
to-report sigmoid [x]
  let i 0
  let output []
  while [i < length x] [
    set output lput (1 / (1 + exp (0 - (item i x)))) output
    set i i + 1
  ]
  report output
end
...
;Calculates the values of the nodes of the next layer
to-report feedforward [a biases weights]
  report sigmoid (matrix-addition biases (matrix-multiplication a weights))
end
...
to neural-network
  ask robots [
    show inputs
    let layer1 feedforward inputs b0 w0
    ;show layer1

    let layer2 feedforward layer1 b1 w1
    ;show layer2

    let layer3 feedforward layer2 b2 w2
    ;show layer3

    let cone-output last layer3

    let output but-last layer3

    if item 0 output = max output [moveForward]
